...
Local release checks use two compiler jobs and two test threads. Development/test debug information and incremental compilation are disabled, matching CI. An isolated Cargo home and build cache persist under `target/release-check/`; the registry cache is shared when available. Cargo runs from a neutral temporary working directory so personal and project Cargo configuration cannot inject dependency patches. The first check populates the build cache; subsequent checks reuse it. These settings reduce memory pressure but do not impose a hard memory limit.
