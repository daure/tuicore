...
impl<M> Toggle<M> {
    ...
    fn line(&self) -> Line<'static> {
        ...
        let mut spans = vec![
            ...
            Span::styled(
                self.label.clone(),
                Style::default().fg(self.text_color.value()),
            ),
        ];
        ...
        if let Some(ref hotkey) = self.hotkey {
            spans.extend([
                ...
                Span::styled(
                    format!("({hotkey})"),
                    Style::default().fg(self.hotkey_color.value()),
                ),
            ]);
        }
    ...
    M: 'static,
{
    ...
    fn layout(&mut self, area: Rect, ctx: &mut LayoutCtx) -> LayoutResult {
        if let Some(hotkey) = self.hotkey_event() {
            ctx.register_focusable_with_hotkey(FocusId::new(TOGGLE_FOCUS), area, true, hotkey);
        } else {
