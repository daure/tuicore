...
mod tests {
    ...
    #[test]
    fn focused_toggle_uses_accent_role_and_bold_switch_only() {
        let focused = Toggle::<()>::new("Telemetry").focused(true).line();
        let unfocused = Toggle::<()>::new("Telemetry").line();

        assert_eq!(focused.spans[0].style.fg, Some(theme().accent_fg()));
        assert_eq!(focused.spans[2].style.fg, Some(theme().accent_fg()));
